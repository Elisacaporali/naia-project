...
class AssistenteIA:
    ...
    def organizar_tarefas(self):
        ...
        # Sugestões específicas para cada tipo de tarefa
        sugestoes = {
            "trabalho": self.sugerir_trabalho,
            "estudo": self.sugerir_estudo,
            "casa": self.sugerir_casa,
            "exercício": self.sugerir_exercicio,
            "lazer": self.sugerir_lazer
        }
        
        # Encontrar a categoria mais próxima da tarefa inserida
        categoria = min(sugestoes.keys(), key=lambda x: self.levenshtein(x, nova_tarefa.lower()))
        
        # Chamar a função de sugestão apropriada
        sugestoes[categoria](nova_tarefa)
    ...
    def sugerir_trabalho(self, tarefa):
        tipo_trabalho = self.fazer_pergunta(f"Qual tipo de trabalho você precisa focar para '{tarefa}'?")
        if "ligar" in tipo_trabalho.lower() or "cliente" in tipo_trabalho.lower():
            nome_cliente = self.fazer_pergunta("Qual é o nome do cliente?")
            numero_cliente = self.fazer_pergunta("Qual é o número de telefone do cliente?")
            horario = self.fazer_pergunta("Qual horário você planeja fazer a ligação?")
            print(f"Assistente: Ótimo! Vou adicionar à sua lista de tarefas: Ligar para {nome_cliente} ({numero_cliente}) às {horario} para {tarefa}.")
        else:
            tempo_estimado = self.fazer_pergunta(f"Quanto tempo você estima que levará para completar '{tipo_trabalho}'?")
            print(f"Assistente: Entendi. Vou adicionar à sua lista: {tipo_trabalho} para {tarefa}, com duração estimada de {tempo_estimado}.")

    def sugerir_estudo(self, tarefa):
        materia = self.fazer_pergunta(f"Qual matéria você precisa estudar para '{tarefa}'?")
        tempo_estudo = self.fazer_pergunta("Por quanto tempo você planeja estudar?")
        print(f"Assistente: Ótimo! Vou adicionar à sua lista: Estudar {materia} por {tempo_estudo} para {tarefa}.")
        print("Assistente: Lembre-se de fazer pausas regulares e usar técnicas como o método Pomodoro para manter o foco.")

    def sugerir_casa(self, tarefa):
        detalhe = self.fazer_pergunta(f"Algum detalhe específico sobre a tarefa '{tarefa}'?")
        print(f"Assistente: Entendi. Vou adicionar à sua lista: {tarefa} - {detalhe}.")
        print("Assistente: Tente dividir essa tarefa em etapas menores se for muito grande. Isso tornará mais fácil começar e manter o progresso.")

    def sugerir_exercicio(self, tarefa):
        tipo_exercicio = self.fazer_pergunta(f"Que tipo de exercício você planeja fazer para '{tarefa}'?")
        duracao = self.fazer_pergunta("Por quanto tempo você planeja se exercitar?")
        print(f"Assistente: Ótimo! Vou adicionar à sua lista: {tipo_exercicio} por {duracao} para {tarefa}.")
        print("Assistente: Lembre-se de se hidratar bem e fazer um aquecimento adequado antes de começar.")

    def sugerir_lazer(self, tarefa):
        atividade = self.fazer_pergunta(f"Que atividade de lazer você planeja para '{tarefa}'?")
        com_quem = self.fazer_pergunta("Você fará essa atividade sozinho ou com alguém?")
        print(f"Assistente: Que legal! Vou adicionar à sua lista: {atividade} {com_quem} para {tarefa}.")
        print("Assistente: Aproveite esse momento para relaxar e recarregar suas energias.")
